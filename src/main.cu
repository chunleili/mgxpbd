...
void substep_all_solver()
{
    ...
    for (int iter = 0; iter <= max_iter; iter++)
    {
        ...
        tic();
        fill_A_omp_triplet();
        toc("fill_A_omp_triplet");

        tic();
        fill_A_omp_coeffRef();
        toc("fill_A_omp_coeffRef");

        tic();
        fill_A_by_coeffRef();
        toc("fill_A_by_coeffRef");
